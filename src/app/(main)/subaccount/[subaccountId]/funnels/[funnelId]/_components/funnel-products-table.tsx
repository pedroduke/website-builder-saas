...
const FunnelProductsTable: React.FC<FunnelProductsTableProps> = ({ products, defaultData }) => {
  ...
  const handleSaveProducts = async () => {
    ...
    toast({
      title: 'Success',
      description: 'The products changes were saved!',
    });
  ...
  return (
    <>
      ...
      <Button disabled={isLoading} onClick={handleSaveProducts} className="mt-4 text-white">
        {isLoading ? (
          <>
            <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving Products...
          </>
        ) : (
          'Save Products'
        )}
